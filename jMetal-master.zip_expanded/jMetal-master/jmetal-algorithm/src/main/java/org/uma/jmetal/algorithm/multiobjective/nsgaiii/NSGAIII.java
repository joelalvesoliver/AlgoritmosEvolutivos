...
public class NSGAIII<S extends Solution<?>> extends AbstractGeneticAlgorithm<S, List<S>> {
  ...
  public NSGAIII(NSGAIIIBuilder<S> builder) { // can be created from the NSGAIIIBuilder within the same package
    ...
    printReferenceToFile("VARReferenceDTLZ1",referencePoints);
...
@Override
protected List<S> createInitialDESurrogate(int variaveis, int solucoes) {
	// TODO Auto-generated method stub
	return null;
}

@Override
protected List<S> evaluateSurogateDE(List<S> population) {
	// TODO Auto-generated method stub
	return null;
}

@Override
protected double avaliation(List<S> population) {
	// TODO Auto-generated method stub
	return 0;
}

@Override
protected List<S> Uniao(List<S> population, List<S> population1) {
	// TODO Auto-generated method stub
	return null;
}

@Override
protected List<S> getLideres(List<S> population) {
	// TODO Auto-generated method stub
	return null;
}

public void printReferenceToFile(String context, List<ReferencePoint<S>> solutionList) {
	FileOutputContext varFileContext = new DefaultFileOutputContext(context);
	BufferedWriter bufferedWriter = varFileContext.getFileWriter();

    try {
      if (solutionList.size() > 0) {
        int numLinhas = solutionList.get(0).MemberSize();
        for (int i = 0; i < solutionList.size(); i++) {
        	List<Double> sref = solutionList.get(i).pos();
        	for (int j = 0; j < sref.size(); j++) {
            bufferedWriter.write(sref.get(j).toString() + "\t");
          }
